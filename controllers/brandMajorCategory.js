...
exports.editBrandMajorCategory = async (req, res) => {
    try {
        const editbrandmajorCategory = await brandMajorCategoryModel.findOneAndUpdate({ brandCategory_id: req.body.brandCategory_id }, {
        ...
        res.status(200).send({ success: true, data: editbrandmajorCategory })
    } catch (err) {
        res.status(500).send({
            error: err,
